...
pub fn ioremap32_read_swap(addr:u64)->Result<u32,String> {
...
pub fn ioremap32_write_swap(addr:u64, write_val:u32)->Result<(),String>{
    let sw;
    if cfg!(target_endian = "big") {
        sw = write_val.to_le();
    } else {
...
pub fn ioremap32_write(addr:u64, write_val:u32)->Result<(),String>{
    if *MEM_FD < 0 {
        return Err("handle of /dev/mem is wrong".to_owned());
    }
    let mask = (4096-1) as u64;
    let ofst= addr as u64 & mask;
    let page = addr as u64 & !mask;
    let len=4;

    unsafe {
        //#[cfg(any(target_arch = "aarch64", target_arch="x86_64"))]
        cfg_if::cfg_if! {
            if #[cfg(any(feature = "mmap64"))] {
            ...
            else if #[cfg(feature = "mmap_libc32")] {
...
pub fn ioremap32_read(addr:u64)->Result<u32,String> {
    if *MEM_FD < 0 {
        return Err("handle of /dev/mem is wrong".to_owned());
    }
    let mask = (4096-1) as u64;
